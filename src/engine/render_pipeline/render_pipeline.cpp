...
void RenderPipeline::RenderLights() const {
    ...
    for (const auto& value : _lights2d)
    {

        if (const auto& light = value.lock())
        {
            light->Update(); // move to material

            const auto model = light->GetEntity().lock()->GetComponent<Transform>().lock();

            const auto shaderId = light->GetShaderId();
