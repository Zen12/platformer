...
float sampleSpotShadowMap(int index, vec2 uv)
{
    if (index == 0) return texture(spotShadowMaps[0], uv).r;
    else if (index == 1) return texture(spotShadowMaps[1], uv).r;
    else if (index == 2) return texture(spotShadowMaps[2], uv).r;
    else return texture(spotShadowMaps[3], uv).r;
}

vec2 spotShadowMapTexelSize(int index)
{
    if (index == 0) return 1.0 / vec2(textureSize(spotShadowMaps[0], 0));
    else if (index == 1) return 1.0 / vec2(textureSize(spotShadowMaps[1], 0));
    else if (index == 2) return 1.0 / vec2(textureSize(spotShadowMaps[2], 0));
    else return 1.0 / vec2(textureSize(spotShadowMaps[3], 0));
}
...
float calcSpotShadow(int index, vec3 worldPos)
{
    ...
    float bias = 0.002;
    float shadow = 0.0;
    vec2 texelSize = spotShadowMapTexelSize(index);

    for (int x = -1; x <= 1; ++x) {
        for (int y = -1; y <= 1; ++y) {
            float pcfDepth = sampleSpotShadowMap(index, projCoords.xy + vec2(float(x), float(y)) * texelSize);
            shadow += currentDepth - bias > pcfDepth ? 1.0 : 0.0;
        }
    }
    shadow /= 9.0;
...
void main()
{
    ...
    vec3 lit = tintedColor.rgb * (1.0 - shadow) + tintedColor.rgb * spotContribution;
    FragColor = vec4(lit, tintedColor.a);
}
